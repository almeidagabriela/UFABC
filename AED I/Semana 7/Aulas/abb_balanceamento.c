...
NoArvore *balancear(NoArvore *raiz) {
    ...
    //Se o modulo de fb for maior que 1 = desbalanceado
    if (fb > 1) {
        // fb > 1 = subarvore esquerda mais alta

        //Verificar se há a necessidade de rotação dupla
    ...
    if (fb < -1) {
        // fb > 1 = subarvore direita mais alta
...
int main() {
    ...
    /*
    Ao implementar a função em um projeto,
    chame-a para todos os ancestrais do nó alterado

    --aqui é só um exemplo da função de forma isolada--
    */
    tree = balancear(tree);
